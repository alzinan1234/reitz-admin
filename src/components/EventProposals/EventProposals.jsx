    ...
    <div className="min-h-screen bg-white p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Search events..." className="pl-9 pr-4 py-2 border rounded text-sm w-64" />
        </div>
      </div>
            ...
            {events.map((event) => (
              <tr key={event.id} className="hover:bg-gray-50 text-sm">
                ...
                <td className="py-4 px-6 flex justify-center gap-3">
                  <button onClick={() => handleAction(event.id, 'approve')} className="p-2 bg-green-500 text-white rounded-full"><CheckIcon /></button>
                  <button onClick={() => handleAction(event.id, 'reject')} className="p-2 bg-red-500 text-white rounded-full"><CrossIcon /></button>
                  <button onClick={() => router.push(`/admin/event-proposals/${event.id}`)} className="p-2 bg-purple-500 text-white rounded-full"><EyeIcon /></button>
                </td>
              </tr>
            ))}
          </tbody>
