      ...
      >
        <div className="flex flex-col h-full justify-between border-r border-[#D6D6D6]">
          ...
          <div className="flex items-center justify-between border-b border-[#D6D6D6] pb-4 p-[10px]">
           <img className="w-[51px] h-[50px]" src="/login-image-90.png" alt="" />
            ...
            {navItems.map(({ name, href, icon: Icon }) => {
              ...
              return (
                <Link
                  key={name}
                  href={href}
                  // Updated active state with gradient colors
                  className={`flex items-center px-4 w-[218px] mx-auto py-2 transition-all rounded ${
                    isActive
                      ? "bg-gradient-to-r from-[#86ACC8] via-[#557F9E] to-[#4B697F] text-white" // Active state with gradient
                      : "hover:bg-gray-100" // Hover to light gray
                  }`}
                >
