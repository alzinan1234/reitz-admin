...
// import { API_ENDPOINTS } from "@/config/api"; // Adjust path based on your folder structure
...
const navItems = [
  ...
  { name: "Event Proposals", href: "/admin/event-proposals", icon: CalendarPlus },
  { name: "Earning", href: "/admin/earning", icon: CircleDollarSign },
...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // 1. Call the API to blacklist the token (based on image_272861.png)
      if (refreshToken && accessToken) {
        await axios.post(
          `${process.env.NEXT_PUBLIC_API_BASE_URL || "https://atm-reply-cast-shipment.trycloudflare.com"}/api/auth/logout/`,
      ...
      >
        <div className="flex flex-col h-full justify-between border-r border-[#D6D6D6]">
          <div className="flex items-center justify-between border-b border-[#D6D6D6] pb-4 p-[10px]">
            <img className="w-[51px] h-[50px]" src="/login-image-90.png" alt="Logo" />
            <button
              onClick={() => setIsOpen(false)}
              className="p-1 hover:bg-gray-100 rounded text-[#494949]"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="21" height="8" viewBox="0 0 21 8" fill="none">
                <path d="M1.5 1H19.5" stroke="#494949" strokeWidth={1.5} strokeLinecap="round" />
                <path d="M1.5 7H19.5" stroke="#494949" strokeWidth={1.5} strokeLinecap="round" />
              </svg>
            </button>
          </div>
          ...
          <nav className="mt-4 space-y-6 flex-grow overflow-y-auto">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center px-4 w-[218px] mx-auto py-2 transition-all rounded ${
                    isActive
                      ? "bg-gradient-to-r from-[#86ACC8] via-[#557F9E] to-[#4B697F] text-white"
                      : "hover:bg-gray-100"
                  }`}
                >
            ...
            <div className="border-t border-[#D6D6D6] pt-6 ">
              <button 
                ...
                className="flex ml-9 gap-2 items-center text-[#FF0000] hover:text-red-600 transition-colors"
              >
